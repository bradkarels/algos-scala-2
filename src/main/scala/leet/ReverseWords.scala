 ...
 */
object ReverseWords {
  // leetcode not allowing 'isBlank'...so rolling my own...
  def myIsBlank(s: String): Boolean = s.indexWhere(c => !c.isWhitespace) == -1

  def reverseWords(s: String): String = {
    var str = s
    var res = ""
    while (str.nonEmpty) {
      val tmp = str.dropWhile(_ == ' ')
      val wrd = tmp.takeWhile(_ != ' ')
      res = wrd + res
      str = tmp.dropWhile(_ != ' ')
      if (!myIsBlank(str)) res = " " + res
    }
    res
  }
}
